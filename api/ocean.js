// ============================================================
// api/ocean.js  —  GET /api/ocean?date=YYYY-MM-DD
// Returns SST grid, isotherms, temp breaks for the map
// ============================================================
  ...
  try {
    ...
    if (!row) {
      return res.status(404).json({
        error: 'No ocean data available. Run /api/cron-fetch first.',
      });
    }
    ...
    const response = {
      ...
      chlDate: row.chl_date,
      updatedAt: row.updated_at,
    };

    // Send grids as compact arrays (Float32 → regular array for JSON)
    ...
    // Cache for 1 hour (data is updated daily)
    res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate=86400');
    return res.status(200).json(response);

  } catch(err) {
    console.error('/api/ocean error:', err);
