...
async function fetchMarineForecast(zoneId){
  try{
    // Use NWS text product endpoint - more reliable than API
    const url='https://tgftp.nws.noaa.gov/data/forecasts/marine/coastal/am/'+zoneId.toLowerCase()+'.txt';
    const resp=await fetch(url,{headers:{'User-Agent':'DIALED/1.0'},signal:AbortSignal.timeout(10000)});
    if(!resp.ok) return null;
    const text=await resp.text();
    // Parse the text forecast into periods
    const periods=[];
    const lines=text.split('\n');
    let currentPeriod=null;
    for(const line of lines){
      const trimmed=line.trim();
      // Period headers start with . like .TODAY... or .TONIGHT...
      const hdr=trimmed.match(/^\.([A-Z][A-Z ]+?)\.\.\.(.*)$/);
      if(hdr){
        if(currentPeriod) periods.push(currentPeriod);
        const name=hdr[1].trim();
        const firstLine=hdr[2].trim();
        currentPeriod={name, text:firstLine, parsed:parseMarineForecast(firstLine)};
      } else if(currentPeriod && trimmed && !trimmed.startsWith('$$') && !trimmed.startsWith('AMZ') && !trimmed.match(/^[0-9]/)){
        currentPeriod.text+=' '+trimmed;
        currentPeriod.parsed=parseMarineForecast(currentPeriod.text);
      }
    }
    if(currentPeriod) periods.push(currentPeriod);
    return{zone:zoneId, updated:new Date().toISOString(), periods:periods.slice(0,8)};
  }catch(e){
    console.log('Marine forecast failed:',e.message);
    return null;
  }
}
