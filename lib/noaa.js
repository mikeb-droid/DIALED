...
async function fetchChlorophyll(reqDate){
  // NOAA VIIRS Science Quality global 4km - noaacwNPPVIIRSSQchlaDaily
  ...
  for(let daysBack=2; daysBack<=21; daysBack++){
    try{
      ...
      console.log('Trying VIIRS SQ chl time:',time.slice(0,10));
      const url='https://coastwatch.noaa.gov/erddap/griddap/noaacwNPPVIIRSSQchlaDaily.json?chlor_a[('+time+')][(0):1:(0)][('+LAT0+'):4:('+LAT1+')][('+LON0+'):4:('+LON1+')]';
      const resp=await fetch(url,{headers:{'User-Agent':'DIALED/1.0'},signal:AbortSignal.timeout(15000)});
      if(!resp.ok){console.log('VIIRS SQ chl returned',resp.status);continue;}
      const data=await resp.json();
      const points=data.table.rows
        .filter(r=>r[4]!==null&&r[4]>0&&r[4]<100)
        .map(([time,alt,lat,lon,chl])=>({lat:+lat.toFixed(3),lon:+lon.toFixed(3),chl:+chl.toFixed(3)}));
      if(points.length<10){console.log('VIIRS SQ chl too few:',points.length);continue;}
      console.log('VIIRS SQ chl success:',points.length,'points, date:',time.slice(0,10));
      return{points,date:time.slice(0,10),source:'noaacwNPPVIIRSSQchlaDaily'};
    }catch(e){
      console.log('VIIRS SQ chl error:',e.message);
    }
  }
  throw new Error('Chlorophyll fetch failed');
}
const FL_BUOYS=[
