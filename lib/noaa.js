...
async function fetchChlorophyll(reqDate){
  ...
  // Try only 3 dates with 8 second timeout each = max 24 seconds
  for(let daysBack=2; daysBack<=30; daysBack+=5){
    try{
      const d=reqDate?new Date(reqDate):new Date();
      d.setDate(d.getDate()-daysBack);
      const time=d.toISOString().slice(0,10)+'T12:00:00Z';
      console.log('Chl try:',time.slice(0,10));
      ...
      const resp=await fetch(url,{headers:{'User-Agent':'DIALED/1.0'},signal:AbortSignal.timeout(8000)});
      if(!resp.ok){console.log('Chl',resp.status);continue;}
      const data=await resp.json();
      const points=data.table.rows.filter(r=>r[4]!==null&&r[4]>0&&r[4]<100).map(([t,a,lat,lon,chl])=>({lat:+lat.toFixed(3),lon:+lon.toFixed(3),chl:+chl.toFixed(3)}));
      if(points.length<10){console.log('Chl too few:',points.length);continue;}
      console.log('Chl success:',points.length,'pts',time.slice(0,10));
      return{points,date:time.slice(0,10),source:'noaacwNPPVIIRSSQchlaDaily'};
    }catch(e){console.log('Chl err:',e.message);}
  }
  throw new Error('Chl all failed');
}
const FL_BUOYS=[
