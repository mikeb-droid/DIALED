...
async function fetchChlorophyll(reqDate){
  ...
  const resp=await fetch(erddapUrl('nesdisVHNSQchlaWeekly','chlor_a',time,4),{headers:{'User-Agent':'DIALED/1.0'},signal:AbortSignal.timeout(45000)});
...
const MARINE_ZONES=[{id:'AMZ632',name:'Jacksonville to Flagler 20-60nm'},{id:'AMZ652',name:'Flagler to Jupiter 20-60nm'},{id:'AMZ672',name:'Jupiter to Deerfield 20-60nm'}];
async function fetchMarineForecast(zoneId){
  try{
    const resp=await fetch('https://api.weather.gov/zones/offshore/'+zoneId+'/forecast',{headers:{'User-Agent':'DIALED/1.0','Accept':'application/json'},signal:AbortSignal.timeout(10000)});
    if(!resp.ok)return null;
    const data=await resp.json();
    const periods=data.properties&&data.properties.periods||[];
    return{zone:zoneId,updated:data.properties&&data.properties.updated,periods:periods.slice(0,4).map(p=>({name:p.name,text:p.detailedForecast,parsed:parseMarineForecast(p.detailedForecast)}))};
  }catch(e){return null;}
}
