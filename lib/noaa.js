...
async function fetchChlorophyll(reqDate){
  ...
  const datasets=[
    // NOAA East Coast CoastWatch ERDDAP - VIIRS chlorophyll FL-GA region
    {id:'VRSUCW_DAILY_SNPP_CHLORA_FG', server:'https://eastcoast.coastwatch.noaa.gov/erddap/griddap', variable:'chlor_a', stride:2},
    // NOAA East Coast CoastWatch ERDDAP - SE region  
    {id:'VRSUCW_DAILY_SNPP_CHLORA_SE', server:'https://eastcoast.coastwatch.noaa.gov/erddap/griddap', variable:'chlor_a', stride:2},
    // NOAA VIIRS NRT 750m Sector UZ
    {id:'noaacwNPPVIIRSchlaSectorUZDaily', server:'https://coastwatch.pfeg.noaa.gov/erddap/griddap', variable:'chlor_a', stride:2},
    // NOAA VIIRS NRT daily global 4km
    {id:'noaacwNPPVIIRSchlaDaily', server:'https://coastwatch.noaa.gov/erddap/griddap', variable:'chlor_a', stride:4},
  ];
  
  for(const ds of datasets){
    // Try up to 14 days back to find valid data
    for(let daysBack=8;daysBack<=14;daysBack++){
    try{
