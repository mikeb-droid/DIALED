...
function aDep(lon,lat){
  const off=Math.max(0,lon-coastLon(lat));if(off<0.001)return 0;
  const sb=shelfBreak(lat),p=off/sb;
  if(p<0.05)return p/0.05*25;if(p<0.25)return 25+(p-0.05)/0.20*55;
  if(p<0.60)return 80+(p-0.25)/0.35*70;if(p<0.95)return 150+(p-0.60)/0.35*50;
  if(p<1.10)return 200+(p-0.95)/0.15*400;if(p<1.60)return 600+(p-1.10)/0.50*600;
  if(p<2.50)return 1200+(p-1.60)/0.90*1800;if(p<4.00)return 3000+(p-2.50)/1.50*2000;
  return 5000;
}
function buildDepthGrid(){
  const DG=new Float32Array(NX*NY),GG=new Float32Array(NX*NY);
...
function interpolateToGrid(points,DG,field){
  if(!points||points.length===0)return null;
  const grid=new Float32Array(NX*NY);
  for(let iy=0;iy<NY;iy++)for(let ix=0;ix<NX;ix++){
    if(DG[iy*NX+ix]<=0)continue;
    const lon=GL0+ix/NX*(GL1-GL0),lat=GA1-iy/NY*(GA1-GA0);
    const nearest=points.map(p=>({v:p[field],d2:(p.lon-lon)**2+(p.lat-lat)**2})).sort((a,b)=>a.d2-b.d2).slice(0,6);
    let wSum=0,vSum=0;
    for(const {v,d2} of nearest){const w=d2<1e-8?1e8:1/d2;wSum+=w;vSum+=w*v;}
...
function gvBil(G,lon,lat){
  ...
  const v00=G[iy*NX+ix],v10=G[iy*NX+(ix+1)],v01=G[(iy+1)*NX+ix],v11=G[(iy+1)*NX+(ix+1)];
  if(!v00||!v10||!v01||!v11)return G[iy*NX+ix]||0;
  return v00*(1-tx)*(1-ty)+v10*tx*(1-ty)+v01*(1-tx)*ty+v11*tx*ty;
}
function smoothGrid(G, NX, NY, passes=4){
  // Gaussian-like blur to remove column-wise SST artifacts
  let src=new Float32Array(G);
  for(let p=0;p<passes;p++){
    const dst=new Float32Array(src.length);
    for(let iy=2;iy<NY-2;iy++) for(let ix=2;ix<NX-2;ix++){
      const v=src[iy*NX+ix];
      if(!v){dst[iy*NX+ix]=0;continue;}
      // 5x5 weighted blur - strongly horizontal to fix vertical artifacts
      let sum=0,wt=0;
      for(let dy=-2;dy<=2;dy++) for(let dx=-2;dx<=2;dx++){
        const sv=src[(iy+dy)*NX+(ix+dx)];
        if(!sv) continue;
        // Weight horizontal neighbors more than vertical
        const w=1.0/(1+Math.abs(dx)*0.5+Math.abs(dy)*2.0);
        sum+=sv*w; wt+=w;
      }
      dst[iy*NX+ix]=wt>0?sum/wt:v;
    }
    src=dst;
  }
  return src;
...
function computeIsotherms(SG){
  if(!SG)return[];
  const SGS=smoothGrid(SG,NX,NY,3); // smooth before marching squares
  const cW=(GL1-GL0)/NX,cH=(GA1-GA0)/NY,result=[];
  const ST=3; // stride - subsample grid to remove column artifacts
  for(let T=68;T<=87;T++){
    const major=T%2===0,segs=[],labels=[];
    for(let iy=0;iy<NY-ST;iy+=ST)for(let ix=0;ix<NX-ST;ix+=ST){
      const s00=SGS[iy*NX+ix],s10=SGS[iy*NX+(ix+ST)],s01=SGS[(iy+ST)*NX+ix],s11=SGS[(iy+ST)*NX+(ix+ST)];
      ...
      const lon=GL0+ix*cW,lat=GA1-iy*cH,lx=cW*ST,ly=cH*ST;
...
function computeTempBreaks(SG,DG){
  if(!SG)return[];
  const THRESH=0.65,breaks=[];
  for(let iy=1;iy<NY-1;iy++)for(let ix=1;ix<NX-1;ix++){
    const s=SG[iy*NX+ix];if(!s)continue;
    const sr=SG[iy*NX+(ix+1)],sl=SG[iy*NX+(ix-1)],su=SG[(iy-1)*NX+ix],sd=SG[(iy+1)*NX+ix];
    if(!sr||!sl||!su||!sd)continue;
    const mag=Math.sqrt(((sr-sl)/2)**2+((su-sd)/2)**2);
    if(mag<THRESH||DG[iy*NX+ix]<=0)continue;
    breaks.push({lon:+(GL0+(ix/NX)*(GL1-GL0)).toFixed(3),lat:+(GA1-(iy/NY)*(GA1-GA0)).toFixed(3),intensity:+(Math.min(1,(mag-THRESH)/2.0)).toFixed(2)});
  }
  return breaks;
}
module.exports={NX,NY,GL0,GL1,GA0,GA1,coastLon,shelfBreak,aDep,buildDepthGrid,interpolateToGrid,gvBil,computeIsotherms,computeTempBreaks};
