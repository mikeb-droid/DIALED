...
function smoothGrid(G, NX, NY, passes=2){
  // Simple box blur to remove interpolation artifacts
  const S=new Float32Array(G.length);
  for(let p=0;p<passes;p++){
    const src=p===0?G:S;
    for(let iy=1;iy<NY-1;iy++) for(let ix=1;ix<NX-1;ix++){
      const v=src[iy*NX+ix];
      if(!v){S[iy*NX+ix]=0;continue;}
      const n=src[(iy-1)*NX+ix],s=src[(iy+1)*NX+ix];
      const e=src[iy*NX+(ix+1)],w=src[iy*NX+(ix-1)];
      // Only smooth if all neighbors are non-zero (ocean cells)
      if(n&&s&&e&&w) S[iy*NX+ix]=(v*4+n+s+e+w)/8;
      else S[iy*NX+ix]=v;
    }
  }
  return S;
