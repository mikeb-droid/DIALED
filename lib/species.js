...
export function computeWaypoints(fish, SG, CG, DG, GG, lunarDate=new Date()) {
  ...
  function cLon(lat) {
    // Accurate FL east coast (verified coordinates)
    if(lat>=31.0) return -81.47;
    if(lat>=30.7) return -81.47+(lat-31.0)*0.30;
    if(lat>=30.4) return -81.38;
    if(lat>=30.0) return -81.38+(lat-30.4)*0.25;
    if(lat>=29.5) return -81.28+(lat-30.0)*0.30;
    if(lat>=29.0) return -81.13+(lat-29.5)*0.32;
    if(lat>=28.5) return -80.97+(lat-29.0)*0.44;
    if(lat>=28.0) return -80.75+(lat-28.5)*0.30;
    if(lat>=27.5) return -80.60+(lat-28.0)*0.32;
    if(lat>=27.0) return -80.44+(lat-27.5)*0.44;
    if(lat>=26.5) return -80.22+(lat-27.0)*0.30;
    if(lat>=26.0) return -80.07;
    if(lat>=25.5) return -80.07+(lat-26.0)*(-0.10);
    if(lat>=25.0) return -80.12+(lat-25.5)*(-0.46);
    return -80.35;
  }
