...
function computeWaypoints(fish,SG,CG,DG,GG,lunarDate){
  ...
  return picked.map(w=>({letter:w.letter,lat:+w.lat.toFixed(3),lon:+w.lon.toFixed(3),dep:Math.round(w.dep),sst:+w.sst.toFixed(1),chl:+w.chl.toFixed(2),gs:+w.gs.toFixed(2),score:{total:+w.score.total.toFixed(2),sst:+w.score.ss.toFixed(2),chl:+w.score.cs.toFixed(2),dep:+w.score.ds.toFixed(2),lunar:+w.score.ls.toFixed(2)}}));
}
