...
export const FISH = [
  ...
  {id:'king',minOff:0.05,maxOff:0.30, name:'Kingfish',     icon:'👑', c:'#ffdd00',
  ...
  {id:'cobia',minOff:0.02,maxOff:0.25,name:'Cobia',        icon:'🦈', c:'#88dd00',
  ...
  {id:'snap',minOff:0.05,maxOff:0.50, name:'Snapper',      icon:'🔴', c:'#ff7777',
  ...
  {id:'grp',minOff:0.05,maxOff:0.50,  name:'Grouper',      icon:'🟫', c:'#dd9933',
  ...
  {id:'aj',minOff:0.08,maxOff:0.60,   name:'Amberjack',    icon:'⚓', c:'#ffaa44',
  ...
  {id:'tarp',minOff:0.01,maxOff:0.10, name:'Tarpon',       icon:'🥈', c:'#aabbdd',
  ...
  {id:'flnd',minOff:0.01,maxOff:0.08, name:'Flounder',     icon:'🫓', c:'#ccbb77',
...
export function computeWaypoints(fish, SG, CG, DG, GG, lunarDate=new Date()) {
  console.log('[species.js v12] computing waypoints for', fish.id, 'minOff:', fish.minOff, 'maxOff:', fish.maxOff);
  if(!SG||!DG) return [];
  const STEP=0.15;
  ...
  for(let lat=GA0+0.2;lat<Math.min(GA1-0.2,31.4);lat+=STEP) {
    for(let lon=Math.max(GL0+0.2, cLon(lat)+0.30);lon<GL1-0.2;lon+=STEP) {
      const offDist = lon - cLon(lat);
      if(lon < cLon(lat) + 0.30) continue;  // hard floor: must be 0.30° offshore
if(offDist < (fish.minOff||0.30)) continue;
      if(offDist > (fish.maxOff||3.0)) continue;
      const sst=gvBil(SG,lon,lat);
      if(!sst || sst < 68) continue;
      const chl=gvBil(CG,lon,lat),gs=gvBil(GG,lon,lat);
      if(chl <= 0) continue;
      const dep=gvBil(DG,lon,lat);
...
// deployed Mon May 18 15:55:19 EDT 2026
